...
contract DaoTreasury is Ownable {
    ...
    address public sFAO;
    uint public sFAOQueue; // Delays change to sFAO address
    ...
    /**
        @notice allow approved address to deposit an asset for FAO
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint FAO needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( FAO ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn FAO for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOfToken( _token, _amount );
        IFAOERC20( FAO ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sFAO ).balanceOf( msg.sender ); // Can only borrow against sFAO held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with FAO
        @param _amount uint
     */
    function repayDebtWithFAO( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IFAOERC20( FAO ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, FAO, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( FAO ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( FAO ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns FAO valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOfToken( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match FAO decimals
            value_ = _amount.mul( 10 ** IERC20( FAO ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SFAO ) { // 9
            sFAOQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
